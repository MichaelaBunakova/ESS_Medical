...
|-------|-------------------|-----------------|
| Model 2 | 4.135 | Male, age 40–49, less than post-secondary education, manager/professional, highest income quintile, 2-person household, working 36–55 hrs/week, partner working 36–55 hrs/week |
...
- **Chart and table side by side:** Each panel displays the bar chart on the left (55% width) and the summary table on the right (45% width), separated by a thin vertical rule, enabling direct comparison of visual and numeric information without scrolling
- **Table row striping:** Rows alternate between transparent and a subtle tinted background using a simple global row counter that increments for every data row, regardless of section boundaries. This avoids the visual artefact of adjacent sections accidentally sharing the same background colour when a section contains an odd number of rows
- **Section headers** in the table use a thin top border and italic text to delineate covariate groups without disrupting the alternating stripe sequence
- **Models 3 and 4** show only newly added variables (welfare regime / GDP tertile) plus the updated MP partner coefficient, labelled with an amber "new" badge, rather than repeating all Model 2 covariates

---

## Reference categories

| Covariate group | Reference category |
|----------------|-------------------|
| Age | 40–49 years |
| Gender | Male |
| Own education | Less than post-secondary |
| Mother's education | Less than post-secondary |
| Occupation | Managers/professionals |
| Household income | Highest quintile |
| Household size | 2 members |
| Respondent working hours | 36–55 hrs/week |
| Partner working hours | 36–55 hrs/week |
| Welfare regime (Model 3) | Post-communist |
| GDP tertile (Model 4) | Q1 (lowest) |
...
**95% CI whiskers** — drawn as a custom `afterDatasetsDraw` plugin:
...
**Table row striping** — applied via inline styles using a simple global counter that increments for every data row, independent of section headers:

```javascript
let dataRowIndex = 0;
rows.forEach(r => {
  if (r.section) {
    // render section header row — do not increment counter
  }
  const rowBg = dataRowIndex % 2 === 0 ? 'transparent' : 'rgba(0,0,0,0.035)';
  dataRowIndex++;
  // apply rowBg to all td elements via inline style
});
```
...
- The **wide CI on Lowest income** (SE = 0.172) reflects the sparse representation of MP-partner households at the bottom of the income distribution in this elite-restricted analytical sample — this should be interpreted with caution.
